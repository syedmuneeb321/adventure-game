...
let numHealthpostions:number=3;
let healthPositionHealAmount:number=30;
let healthPostionDropChance:number=50; //percentage
...
const Game=async()=>{
    while(running){
        console.log(`----------------------------------------------`)
        let enemyHealth:number=Math.floor(rand*maxEnemyHealth);
        ...
        while(enemyHealth>0){
            ...
            const {opt}=input;
            if(opt===1){
                let damageDealt:number=Math.floor(rand*attackDamage);
                let damageTaken:number=Math.floor(rand*enemyAttackDamage);
